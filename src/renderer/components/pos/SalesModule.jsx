...
const QuickProducts = ({ products, selectedCategory, handleAddProduct }) => {
  ...
  const filteredProducts = products.filter(p => 
    p.is_active && (selectedCategory === null || p.category_id === selectedCategory)
  );

  const handleProductClick = (product) => {
    if (product.track_inventory && product.current_stock <= 0) return;
  ...
  const handleTouchEnd = () => {
    setTimeout(() => setTouchedProduct(null), 300);
  };

  return (
    <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-2">
      {filteredProducts.map((product) => {
        const isLowStock = product.track_inventory && product.current_stock <= product.low_stock_threshold;
        const isOutOfStock = product.track_inventory && product.current_stock <= 0;
        const wasJustAdded = addedProducts.has(product.id);
        ...
        return (
          ...
          >
            ...
            <div
              className={`
                relative bg-gray-800 border border-gray-700 rounded-lg p-3
                transition-all duration-200
                ${isOutOfStock ? 'opacity-50' : 'hover:border-gray-600'}
                ${wasJustAdded ? 'scale-95 border-green-500' : ''}
              `}
            >
              {/* Stock indicator badge */}
              {product.track_inventory && (
                <div className={`
                  absolute top-2 right-2 text-xs font-medium px-1.5 py-0.5 rounded
                  ${isOutOfStock ? 'bg-red-500/20 text-red-400' : 
                    isLowStock ? 'bg-orange-500/20 text-orange-400' : 
                    'bg-green-500/20 text-green-400'}
                `}>
                  {isOutOfStock ? 'Out' : product.current_stock}
                </div>
              )}
              ...
              <div className="flex justify-center mb-2">
                <div className={`
                  w-12 h-12 bg-gray-700 rounded-lg flex items-center justify-center
                  transition-all duration-300
                  ${wasJustAdded ? 'bg-green-600 scale-110' : ''}
                `}>
                  {wasJustAdded ? (
              ...
              {/* Product name */}
              <h4 className="text-white text-sm font-medium mb-1 line-clamp-2 min-h-[2.5rem] leading-tight">
                {product.name}
              </h4>
              ...
              {/* Price - with better formatting for readability */}
              <div className="mt-auto">
                <p className="text-blue-400 font-bold text-sm" title={`KES ${formatPrice(product.unit_price)}`}>
                  KES {formatPrice(product.unit_price)}
                </p>
                {/* For very large prices, add a subtle indicator */}
                {product.unit_price >= 100000 && (
                  <p className="text-gray-500 text-xs">
                    {product.unit_price >= 1000000 ? 'Million+' : 'Thousand+'}
                  </p>
                )}
              </div>
            </div>

            {/* Add to Cart Button - Always visible, positioned at bottom */}
            <button
              onClick={() => handleProductClick(product)}
              disabled={isOutOfStock}
              className={`
                absolute bottom-0 left-0 right-0 
                bg-blue-600 text-white rounded-b-lg py-2
                flex items-center justify-center gap-1
                transition-all duration-200
                transform translate-y-full
                ${!isOutOfStock && (isTouched || 'group-hover:translate-y-0')}
                ${isOutOfStock ? 'hidden' : ''}
                hover:bg-blue-700 active:bg-blue-800
                ${wasJustAdded ? 'bg-green-600 hover:bg-green-600' : ''}
              `}
            >
              {wasJustAdded ? (
                <>
            ...
            {/* Alternative: Corner Add Button (uncomment to use instead) */}
            {/* <button
              onClick={() => handleProductClick(product)}
              disabled={isOutOfStock}
              className={`
...
// Helper function to format prices - always show full amount for clarity
const formatPrice = (price) => {
  // Always show the full price with proper formatting
  return price.toLocaleString();
};



// Enhanced compact list view with better accessibility and UX
const CompactProductList = ({ products, selectedCategory, handleAddProduct }) => {
  const filteredProducts = products.filter(p =>
    p.is_active &&
      ...
      {filteredProducts.map((product) => {
        ...
        return (
          <button
            ...
            disabled={isOutOfStock}
            aria-label={`Add ${product.name} to cart. Price: ${product.unit_price} KES. ${
              product.track_inventory ? `Stock: ${product.current_stock}` : 'In stock'
            }`}
            className={`
              w-full group relative bg-gray-800 border border-gray-700 rounded-lg
              transition-all duration-200 hover:border-blue-500 hover:shadow-md
              ${isOutOfStock 
                ? 'opacity-60 cursor-not-allowed' 
                : 'cursor-pointer hover:bg-gray-750 active:scale-[0.98]'
              }
            `}
          >
            <div className="flex items-center p-3">
              {/* Product icon with status indicator */}
              <div className="relative flex-shrink-0 mr-3">
                <div className={`
                  w-10 h-10 rounded-lg flex items-center justify-center transition-colors
                  ${isOutOfStock ? 'bg-red-900/30' : 'bg-gray-700 group-hover:bg-gray-600'}
                `}>
                  <Package className={`w-5 h-5 ${isOutOfStock ? 'text-red-400' : 'text-gray-400'}`} />
                </div>
                {/* Stock status dot */}
                {product.track_inventory && (
                  <div className={`
              ...
              <div className="flex-1 min-w-0 text-left">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-gray-400 text-xs">{product.category_name}</span>
                  {product.sku && (
                    <>
              ...
              <div className="flex items-center gap-4 flex-shrink-0">
                {/* Stock indicator with better visibility */}
                {product.track_inventory && (
                  <div className={`
                  ...
                  `}>
                    {isOutOfStock ? 'Out' : `${product.current_stock} left`}
                  </div>
                )}
            ...
            {/* Visual feedback bar on hover */}
            {!isOutOfStock && (
              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left" />
            )}
          </button>
        );
      })}
...
// Professional minimalist card design with improved accessibility
const MinimalProductCards = ({ products, selectedCategory, handleAddProduct }) => {
  const filteredProducts = products.filter(p =>
    p.is_active &&
    (selectedCategory === null || p.category_id === selectedCategory)
  );
      ...
      {filteredProducts.map((product) => {
        ...
        return (
          <button
            ...
            onClick={() => handleAddProduct(product)}
            disabled={isOutOfStock}
            aria-label={`${product.name}, ${product.category_name}, ${formatPrice(product.unit_price)} KES, ${
              product.track_inventory ? `${product.current_stock} in stock` : 'Available'
            }`}
            className={`
              relative bg-gray-800 border border-gray-700 rounded-lg p-3
              transition-all duration-200 hover:border-blue-500 hover:shadow-md
              ...
              group overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900
            `}
          >
            ...
            {/* Product name */}
            <h4 className="text-white text-sm font-medium mb-1 line-clamp-2 leading-tight text-left">
              {product.name}
            </h4>
            
            {/* Category - visible on hover for context */}
            <p className="text-gray-500 text-xs mb-2 truncate opacity-0 group-hover:opacity-100 transition-opacity">
              {product.category_name}
            </p>
            
            {/* Price */}
            <p className="text-blue-400 font-bold text-sm text-left">
              {formatPrice(product.unit_price)}
            </p>

            {/* Visual add indicator on hover/focus */}
            <div className={`
              absolute inset-0 flex items-center justify-center
              bg-gradient-to-t from-blue-600/90 via-blue-600/50 to-transparent
